...
async function uploadFile(base64Data, fileType, acl = "public-read") {
  const params = {
    Bucket: process.env.GETAPLACE_AWS_BUCKET_NAME,
...
function getS3Client() {
  const s3Client = new aws.S3({
    region: process.env.GETAPLACE_AWS_REGION_NAME,
    credentials: {
      accessKeyId: process.env.GETAPLACE_AWS_ACCESS_KEY,
      secretAccessKey: process.env.GETAPLACE_AWS_SECRET_ACCESS_KEY,
    },
  });
  return s3Client;
}
