...
function generateAccessToken(user){
    const token = jwt.sign(user.toJSON(),process.env.access_token,{'expiresIn':'10000000m'});
...
async function registerUser(req,res, userTypeModal){
    ...
    if(!(body.name) || !(body.email) || !(body.password)){
        res.status(400).json({'status':0,'error':"Name Email and Password and Phone Number are required fields"});
        return;
    }
    
    const user = await userTypeModal.find({'$or':[{'email':body.email},{'phoneNumber':body.phoneNumber}]});
    
    if(user.length !=0){
        res.status(400).json({'status' :0 ,'error' :"Email ID/ Phone Number already Registered"});
        return;
    }
...
module.exports = {getErrorValidationMsg,generateAccessToken,validateToken,registerUser,logInUser};